...
export function hasEscapePath(board: BoardSnapshot): boolean {
  const start = board.thief;

  const isInside = (x: number, y: number) => x >= 0 && y >= 0 && x < board.width && y < board.height;
  const isBlockedKey = (k: string) => board.buildings.has(k) || board.police.has(k);
  const isPassable = (x: number, y: number) => isInside(x, y) && !isBlockedKey(`${x},${y}`);
